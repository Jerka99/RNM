var ReactFinger = (function (exports, react) {
  ...
  var isIPadPro = navigator.platform.match("Mac") && (navigator.maxTouchPoints || 0) > 1;
  var isIOS = /(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent) || isIPadPro;
  ...
  var controlKeys = ["control", "meta", "shift", "alt"];
  function isControlKey(key) {
      return controlKeys.indexOf(key) > -1;
  }
  function isMatch(ks, keys) {
      return (ks &&
          keys &&
          keys.length === ks.size &&
          keys.every(function (key) { return ks.has(key.toLowerCase()); }));
  }
  function updateControlKey(ks, key, state) {
      return state ? ks.add(key) : ks.delete(key);
  }
  function updateControlKeys(ks, event) {
  ...
  function useKeySet(flags) {
      var ks = flags.has(KEY_SET)
          ? flags.get(KEY_SET)
          : new Set();
      if (!flags.has(KEY_SET))
          flags.set(KEY_SET, ks);
      return ks;
  }
  function fixMacMetaKeyDown(ks, event) {
      if (event.metaKey || event.key === "Meta") {
          Array.from(ks).forEach(function (it) { return !isControlKey(it) && ks.delete(it); });
      }
  }
  function fixMacMetaKeyUp(ks, event) {
      if (event.metaKey || event.key === "Meta") {
          Array.from(ks).forEach(function (key) { return !isControlKey(key) && ks.delete(key); });
      }
  }
  var FingerShortcutProvider = {
      ...
      handleKeyDown: function (_a) {
          ...
          var flags = context.flags;
          if (!event.repeat)
              flags.set(LAST_ACTION, "down");
          if (flags.get(LAST_ACTION) === "up")
              return;
          var ks = useKeySet(flags);
          fixMacMetaKeyDown(ks, event);
          updateControlKeys(ks, event);
          ks.add(event.key.toLowerCase());
          var when = function (keys, handler) {
              return isMatch(ks, keys) && handler(event);
          };
          var keys = Array.from(ks);
  ...
  var FingerProxy = react.memo(function FingerProxy(props) {
      ...
      var _a = props.target, target = _a === void 0 ? contextTarget || document : _a, _b = props.passive, passive = _b === void 0 ? true : _b, others = __rest(props, ["target", "passive"]);
